...
public class AprioriMyself {
	...
	static List<Mymap> map = new ArrayList<Mymap>();// 存放频繁项集和对应的支持度技术
	...
	public static void main(String args[]) {
		...
		// MIN_SUPPROT + " 最小置信度为：" + MIN_CONFIDENCE);
	...
	private static List<List<String>> getNextCandidate(List<List<String>> FrequentItemset) {
		...
		for (int i = 0; i < FrequentItemset.size(); i++) {
			...
			for (int h = i + 1; h < FrequentItemset.size(); h++) {// 频繁集第i行与第j行(j>i)连接
																	// 每次添加且添加一个元素组成
																	// 新的频繁项集的某一行，
